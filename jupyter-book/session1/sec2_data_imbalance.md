...
공장 불량 탐지 모델의 정확도가 **99%** 라면, 잘 만든 모델일까요?
...
아닙니다. 데이터 구성이 정상 9,900개, 불량 100개라면, 모든 샘플을 정상으로 예측하는 더미 모델조차 99% 정확도를 냅니다. 불량 100개를 전부 놓치면서도요. 이것이 **정확도의 역설(Accuracy Paradox)** 입니다. 두 클래스의 샘플 수가 비슷한 균형 데이터에서는 정확도가 의미 있는 지표로 작동하지만, 제조 현장의 데이터는 그렇지 않습니다.

```{figure} ../../lecture/images/s1_2_img51.jpg
:alt: Figure 1.1 – 균형 잡힌 분포
:width: 67%

Figure 1.1 – 균형 잡힌 분포
```

```{figure} ../../lecture/images/s1_2_img71.jpg
:alt: Figure 1.2 – 불균형 데이터셋
:width: 67%

Figure 1.2 – 불균형 데이터셋
```

```{figure} ../../lecture/images/s1_2_img95.jpg
:alt: Figure 1.3 – 불균형 분류 용어 가이드
:width: 67%

Figure 1.3 – 불균형 분류 용어 가이드
```

불균형 분류에서 쓰는 핵심 용어(다수·소수 클래스, 오버·언더샘플링 등)를 정리한 안내도입니다. 용어를 먼저 정리해 두면 이후 등장할 SMOTE와 CSL 설명이 훨씬 수월하게 읽힙니다.
...
**불균형 데이터셋(Imbalanced Dataset)** 은 타겟 클래스 가운데 일부가 다른 클래스보다 압도적으로 많은 데이터셋을 가리킵니다. 제조업에서는 이것이 일상입니다. 정상 99%, 불량 1%는 양반이고, 불량률이 0.1% 이하인 경우도 흔합니다.
...
```{figure} ../../lecture/images/s1_2_img08.jpg
:alt: Figure 1.4 – ML 알고리즘 결정 경계
:width: 67%

Figure 1.4 – ML 알고리즘 결정 경계
```
...
```{figure} ../../lecture/images/s1_2_img57.jpg
:alt: Figure 1.8 – ROC 곡선
:width: 67%
...
```{figure} ../../lecture/images/s1_2_img97.jpg
:alt: Figure 1.9 – PR 곡선 비교
:width: 67%
...
```{figure} ../../lecture/images/s1_2_img10.jpg
:alt: Figure 2.5 – SMOTE 동작 원리
:width: 67%

Figure 2.5 – SMOTE 동작 원리
```
...
```{figure} ../../lecture/images/s1_2_img27.jpg
:alt: Figure 2.6 – SMOTE 오버샘플링 결과
:width: 67%

Figure 2.6 – SMOTE 오버샘플링 결과
```
...
```{figure} ../../lecture/images/s1_2_img103.jpg
:alt: Figure 2.8 – Borderline-SMOTE
:width: 67%

Figure 2.8 – Borderline-SMOTE
```
...
```{figure} ../../lecture/images/s1_2_img81.jpg
:alt: Figure 2.11 – ADASYN
:width: 67%
...
```{figure} ../../lecture/images/s1_2_img23.jpg
:alt: Figure 5.1 – 비용 민감 학습 방법 분류
:width: 67%

Figure 5.1 – 비용 민감 학습 방법 분류
```
...
```{figure} ../../lecture/images/s1_2_img47.jpg
:alt: Figure 5.6 – 기본 모델 결정 경계
:width: 67%

Figure 5.6 – 기본 모델 결정 경계
```
...
```{figure} ../../lecture/images/s1_2_img24.jpg
:alt: Figure 5.7 – balanced 가중치 적용 후
:width: 67%

Figure 5.7 – balanced 가중치 적용 후
```
...
```{figure} ../../lecture/images/s1_2_img73.jpg
:alt: Figure 5.16 – F1 최적 임계값 PR 곡선
:width: 67%

Figure 5.16 – F1 최적 임계값 PR 곡선
```
...
```{figure} ../../lecture/images/s1_2_img93.jpg
:alt: Figure 5.17 – 임계값별 지표 변화
:width: 67%

Figure 5.17 – 임계값별 지표 변화
```
...
```{figure} ../../lecture/images/s1_2_img41.jpg
:alt: Figure 5.19 – F1 최적 임계값
:width: 67%

Figure 5.19 – F1 최적 임계값
```
