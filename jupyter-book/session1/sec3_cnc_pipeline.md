...
**오늘 만들 파이프라인**: 노이즈 제거 + 평가 지표를 결합하여 마모 시작점을 자동 탐지하는 엔드투엔드 시스템
...
원시 신호를 그대로 모델에 넣으면 **차원의 저주(Curse of Dimensionality)** 에 부딪힙니다. 1초에 10,000개씩 10시간을 수집하면 3억 6천만 개의 입력값이 되기 때문입니다. 교재 3장도 "실제 현장 데이터는 여러 소스에서 수집되며 불완전하고 불일치가 있다"고 지적합니다. 그래서 원시 신호에서 더 유용한 정보를 뽑아내는 **특징 공학(Feature Engineering)** 이 반드시 필요합니다.
...
교재 3장에서는 ROCKET 같은 자동 특징 추출 방법도 소개하지만, 여기서는 직관적으로 이해할 수 있는 위 세 가지를 사용합니다.
...
실제 현장에서는 **레이블이 없는 경우가 더 흔합니다.** "지금까지 공구가 정상이었다"는 알아도 "정확히 몇 시부터 마모가 시작됐는지"에 대한 라벨은 없기 때문입니다. 그래서 여기서는 비지도학습인 **Isolation Forest** 를 사용합니다.

#### Isolation Forest 원리 — "이상값은 고립시키기 쉽다"

Isolation Forest의 직관은 "이상값은 고립시키기 쉽다"는 것입니다. 데이터를 무작위로 분할할 때, 정상 데이터는 밀집해 있어 고립까지 많은 분할이 필요하지만 이상 데이터는 희박해 적은 분할로도 고립됩니다. 따라서 고립에 필요한 평균 분할 수가 곧 이상 점수가 됩니다.
...
시계열 이상은 세 가지로 나뉩니다. **점 이상(Point Anomaly)** 은 개별 포인트가 정상 범위를 벗어난 경우로, 순간 전원 서지나 센서 고장이 그 예입니다. **문맥적 이상(Contextual Anomaly)** 은 값 자체는 정상 범위지만 문맥상 비정상인 경우로, 공구 마모 초기의 미세한 RMS 증가가 여기 해당합니다. **집단 이상(Collective Anomaly)** 은 개별 값은 정상이나 연속된 패턴이 비정상인 경우로, 주파수가 서서히 이동하는 채터링 패턴이 대표적입니다.
...
처리 방식도 둘로 나뉩니다. **배치 처리** 는 전체 데이터를 한 번에 다루는 방식(지금까지의 실습)이고, **스트리밍 처리** 는 데이터가 들어오는 즉시 실시간으로 처리하는 방식입니다. 실시간 처리에는 River 같은 라이브러리를 사용합니다.
...
정리하면 전체 파이프라인은 여섯 단계로 구성됩니다. ① 센서 실시간 스트림을 수집하고, ② Wavelet 노이즈 제거로 전처리한 뒤, ③ 슬라이딩 윈도우(1초, 0.5초 겹침)로 분할하고, ④ RMS·Peak·Kurtosis 특징을 추출한 다음, ⑤ Isolation Forest로 이상 점수를 계산하고, ⑥ 이상 점수가 임계값을 넘으면 경보를 울립니다.
...
교재에서는 이를 "워크-포워드 검증(Walk-Forward Validation)"이라고도 부릅니다.

#### 오차 지표 선택 — 회귀 vs 분류

마지막으로 평가 지표입니다. 이상 점수를 연속값으로 보는 회귀 관점에서는 MSE(오차 제곱 평균, 이상치에 민감), MAE(오차 절댓값 평균, 이상치에 덜 민감), RMSE(MSE의 제곱근, 원래 단위와 같은 스케일)를 사용합니다. 정상/이상 이진 분류 관점에서는 배운 Precision·Recall·F1-Score를 그대로 적용하되, 불균형 데이터이므로 Accuracy는 지양하고 PR Curve와 F1을 권장합니다.

---

## 핵심 요약

1. **노이즈 제거** → **특징 추출**(RMS, Peak, Kurtosis) → **이상탐지**(Isolation Forest)의 6단계 파이프라인
2. 라벨이 없어도 **정상 데이터만으로** 이상탐지 모델 구축 가능
3. 시계열에서는 반드시 **TimeSeriesSplit**으로 교차 검증 (일반 k-fold 금지)
4. 모든 단계가 **배운 이론**과 연결됨
