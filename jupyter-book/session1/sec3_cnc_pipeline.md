...
정리하면 전체 파이프라인은 여섯 단계로 구성됩니다. ① 센서 실시간 스트림을 수집하고, ② 섹션 1의 Wavelet 노이즈 제거로 전처리한 뒤, ③ 슬라이딩 윈도우(1초, 0.5초 겹침)로 분할하고, ④ RMS·Peak·Kurtosis 특징을 추출한 다음, ⑤ Isolation Forest로 이상 점수를 계산하고, ⑥ 이상 점수가 임계값을 넘으면 경보를 울립니다.
...
마지막으로 평가 지표입니다. 이상 점수를 연속값으로 보는 회귀 관점에서는 MSE(오차 제곱 평균, 이상치에 민감), MAE(오차 절댓값 평균, 이상치에 덜 민감), RMSE(MSE의 제곱근, 원래 단위와 같은 스케일)를 사용합니다. 정상/이상 이진 분류 관점에서는 섹션 2에서 배운 Precision·Recall·F1-Score를 그대로 적용하되, 불균형 데이터이므로 Accuracy는 지양하고 PR Curve와 F1을 권장합니다.
