# Autoencoder — 정상을 학습해서 이상을 찾는다

---

## 1-1. 문제 제기

공장에서 **수만 건의 정상**이 생산되는 동안 결함은 수십 건에 불과합니다. 그 수십 건에 정확한 라벨이 붙어 있을 가능성은 거의 없습니다. "이건 분명 결함인데 어떤 종류인지 모르겠다", "정상인가 결함인가 경계가 애매하다", "검사원마다 판정이 다르다" 같은 상황이 현장에서는 흔합니다.
...
지도 학습은 라벨이 있어야 작동합니다. RandomForest든 신경망이든 "이건 정상, 저건 결함"이라는 정답이 있어야 학습할 수 있습니다. 라벨이 없으면 출발 자체가 안 됩니다. 새로운 결함 유형이 나타날 때마다 사람이 일일이 라벨링하는 것은 비현실적이고, 신제품 라인이 가동되면 라벨이 쌓이기도 전에 모델이 필요합니다.
...
라벨 없이, 정상 데이터만으로 이상을 탐지할 수 있을까? 핵심은 **"정상만 학습한 모델은 이상을 재현하지 못한다"** 는 점입니다. 정상 패턴을 압축했다가 복원하는 능력은 키웠지만, 처음 보는 이상 패턴은 복원하지 못합니다. 복원 실패의 크기, 즉 **재구성 오차(reconstruction error)** 가 곧 이상 점수가 됩니다.

```{figure} ../../lecture/images/s2_1_img04.png
:alt: PCA 산점도: 정상과 이상의 분포 차이
:width: 67%

PCA 산점도: 정상과 이상의 분포 차이
```
...
```{figure} ../../lecture/images/s2_1_img06.png
:alt: 비지도 학습 프레임워크 개요
:width: 67%
...
```{figure} ../../lecture/images/s2_1_img07.png
:alt: Autoencoder 재구성 오차 분포: 정상 vs 이상
:width: 67%

Autoencoder 재구성 오차 분포: 정상 vs 이상
```
...
```{figure} ../../lecture/images/s2_1_img08.png
:alt: 임계값에 따른 이상 탐지율 변화
:width: 67%
...
```{figure} ../../lecture/images/s2_1_img09.png
:alt: ROC 커브: 재구성 오차 기반 이상 탐지
:width: 67%

ROC 커브: 재구성 오차 기반 이상 탐지
```
...
```{figure} ../../lecture/images/s2_1_img10.png
:alt: PCA vs Autoencoder 차원 축소 비교
:width: 67%

PCA vs Autoencoder 차원 축소 비교
```
...
```{figure} ../../lecture/images/s2_1_img11.png
:alt: PCA 주성분 분석 결과
:width: 67%
...
```{figure} ../../lecture/images/s2_1_img12.png
:alt: Autoencoder 잠재 공간 시각화
:width: 67%
