...
엔진 초기 (사이클 1~100):
  RUL = 250, 300, 350... 이 구간의 센서 데이터는 사실상 모두 "정상"으로 동일
  → 모델이 서로 다른 RUL 라벨을 구분하려고 헛수고
...
- **음의 상관 센서**: 고장 임박 시 값이 증가 (온도, 압력 등)
- **양의 상관 센서**: 고장 임박 시 값이 감소 (효율, 유량 등)
...
엔진 #1의 노화 패턴:
- **sensor_12**: 시간이 지날수록 서서히 감소하다가 말기에 급격히 떨어짐
- **sensor_11**: 반대로 말기에 급격히 상승

### 심화: 시계열 정상성 검정

![시계열 정상성 검정 결과](../../lecture/images/s2_3_img10.png)

- **정상성(stationarity)**: 통계적 특성이 시간에 따라 변하지 않음
- 검정 방법: ADF (Augmented Dickey-Fuller), KPSS, ACF 분석
- NASA Turbofan의 대부분 센서는 마모 진행에 따라 추세(trend)를 보이는 **비정상 시계열**
- 슬라이딩 윈도우 + LSTM 접근이 유리한 이유
...
- 정상 데이터로만 학습했음에도 노화 구간을 재구성 오차로 구분
- Latent space에서 건강/노화 상태의 분리가 시각적으로 확인됨
...
- 예측값 표준편차가 0에 가까움 → 30 epoch로는 충분히 학습되지 않음
- **교육적으로 좋은 사례**: 모델이 수렴하지 않은 이유, 학습률이나 epoch 수를 어떻게 조정해야 하는지 생각해 볼 것
...
- **Dropout 0.1~0.3** 구간이 일반화에 유리
- **Dropout 0.5**에서는 학습 불충분으로 Test RMSE 악화
- *Deep Learning with Python*: "은행의 모든 직원이 동시에 이직하면 안 된다" — 적절한 비율이 중요
...
- **롤링 통계**: 이동평균, 이동표준편차 (window=5, 10, 30)
- **차분 특징**: 1차/2차 차분으로 추세와 변화율 포착
- **주파수 특징**: FFT 기반 스펙트럼 에너지
- **도메인 특징**: 온도×압력 상호작용, 진동 피크 간격
...
- Autoencoder: **"지금 이상한가?"** 에 답
- LSTM: **"얼마나 더 쓸 수 있는가?"** 에 답
- 두 모델을 통합하면 예지 보전의 두 가지 핵심 질문에 **동시에** 답 가능
