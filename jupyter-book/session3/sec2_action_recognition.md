...
Object Detection은 "무엇이 있는가"를 탐지하고, Action Recognition은 "그 사람이 무엇을 하고 있는가"를 인식합니다. [사람], [안전모], [지게차]처럼 정적인 객체를 찾는 것과, [사람이 난간 없이 높은 곳에서 작업 중]처럼 동적인 행동을 읽어내는 것은 접근 방식 자체가 다릅니다.
...
제조 현장에는 객체 탐지만으로 잡을 수 없는 위험 행동이 많습니다. 2인 1조 작업 규정을 어기고 혼자 중량물을 이동하는 경우, 객체 탐지만으로는 "사람 한 명이 있다"로밖에 보이지 않습니다. 안전 난간 없는 고소 작업, 보호구 미착용 상태의 위험 구역 진입, 허리를 과도하게 굽히는 반복 작업도 같은 한계를 보입니다. 이런 상황을 포착하려면 자세 패턴과 관절 각도를 읽어내야 합니다.
...
- 데이터가 부족할 경우 → 정상 행동만 학습한 **Autoencoder**로 이상탐지 (세션 2 재활용)
```
