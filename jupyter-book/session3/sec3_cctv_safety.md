...
두 모델이 **서로 다른 호흡**으로 동작:

- **YOLO**: 단일 프레임 입력. 매 프레임마다 추론 가능 ("안전모 있다/없다"는 1프레임으로 판단)
- **LSTM**: 30프레임 시퀀스가 모여야 추론. 첫 30프레임은 결과 없음. 호흡이 느림

직렬로 묶으면("YOLO 끝나면 LSTM") → LSTM이 대기 중일 때 YOLO 결과도 같이 멈춤
→ **각 스트림이 자기 속도로 돌아가고, 결과를 OR 조건으로 합쳐서 위반 판정**
...
- **카메라 위치와 각도**: 안전모 탐지에는 머리가 잘 보이는 각도, 행동 인식에는 전신이 잘 보이는 각도 → 약간 높은 위치에 설치하여 한 대로 둘 다 커버
- **클래스 라벨링 규칙의 명확성**: "안전모 미착용"의 경계 케이스(턱끈 안 매면? 머리에 얹기만 하면?)를 미리 정의. 라벨링 가이드라인 문서를 데이터셋과 함께 관리
- **데이터 불균형**: 위반 상황은 정상보다 훨씬 드묾. 그대로 학습하면 "정상"만 외워도 90%대 정확도 → 위반 클래스 오버샘플링 또는 클래스 가중치 조정
...
- 30FPS에서 `min_consecutive=15` → **0.5초 이상 지속**되는 위반만 알림
- `detect_violation` 결과와 함께 사용하면 일시적 오탐 제거
