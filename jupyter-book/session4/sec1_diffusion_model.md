...
제조 현장의 데이터 사정은 가혹합니다. 정상 제품 이미지는 라인이 돌아가는 동안 계속 쌓여 수만 장을 넘기지만, 스크래치 결함은 수백 장, 버블(기포) 결함은 수십 장, 크랙(균열) 결함은 손에 꼽을 정도라 학습 자체가 불가능합니다.
...
Session 1에서 배운 SMOTE를 이미지에 적용할 수 있을까요? 결함 이미지 두 장의 픽셀을 평균 내면 **의미 없는 흐릿한 혼합 이미지**가 나옵니다. 스크래치 반과 버블 반을 섞어봤자 어떤 결함도 아닌 픽셀 덩어리일 뿐입니다. 픽셀 보간으로는 새로운 결함 패턴을 만들 수 없습니다. 우리에게 필요한 건 **진짜처럼 보이면서도 새로운 결함 이미지**입니다.
...
세상에 존재하지 않는 결함 이미지를 진짜처럼 만들어 낼 수 있는가? Session 1과 2에서 마주한 데이터 부족 문제를 **데이터 생성**으로 푸는 접근입니다. Session 1은 SMOTE로 보강했고, Session 3은 임계값 조정으로 대응했지만, Session 4에서는 AI가 학습 데이터 자체를 생성합니다.
