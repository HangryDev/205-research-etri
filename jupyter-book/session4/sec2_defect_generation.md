...
"스크래치 결함 이미지"라고 프롬프트를 주면 무작위 스크래치가 생성됩니다. 어디에, 얼마나 깊게, 어떤 방향인지 제어할 수 없고, 그저 "스크래치가 들어 있는 이미지"가 나올 뿐입니다.

하지만 품질 검사 모델을 학습시키려면 **특정 조건의 결함을 골고루 모아야** 합니다. 스크래치 길이는 1mm, 5mm, 10mm 골고루 필요하고, 깊이는 표면 긁힘부터 깊은 홈까지 다양해야 하며, 방향도 수직, 수평, 대각선을 두루 갖춰야 합니다. 하지만 현장 데이터는 대부분 편향되어 있습니다. 라인 특성상 수직 스크래치가 많고 깊은 스크래치는 거의 없습니다. 이런 편향된 데이터로만 학습한 모델은 본 적 없는 형태의 결함에는 약합니다.

해결책은 부족한 조건의 결함을 텍스트 명세로 정확히 생성하는 것입니다. "스테인리스 스틸 표면 왼쪽 가장자리에 깊이 5mm의 수직 스크래치 결함, 산업용 품질 검사 사진, 고해상도"처럼 한 줄로 결함의 모든 속성을 지정하고 그대로 이미지가 만들어진다면 **데이터셋의 빈 구멍을 정확히 채울 수 있습니다**.

생성된 결함을 원하는 대로 정밀하게 제어할 수 있는가? 그 답이 **프롬프트 기반 조건부 생성**입니다.
