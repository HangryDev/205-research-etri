...
```{admonition} 출처: Diffusion Model
:class: note dropdown
아래 시연/실습은 원래 Diffusion Model** 섹션에 있던 내용입니다.
```
...
이런 한계는 **프롬프트 제어**와 **합성 데이터 검증**에서 보완.
```
...
```{admonition} 출처: Defect Generation
:class: note dropdown
아래 시연/실습은 원래 Defect Generation** 섹션에 있던 내용입니다.
```
...
```{admonition} 출처: 합성 데이터 고도화
:class: note dropdown
아래 시연/실습은 원래 합성 데이터 고도화** 섹션에 있던 내용입니다.
```
